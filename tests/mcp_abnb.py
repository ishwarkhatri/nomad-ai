...
"""An example of the nomad-ai agents incorporating an MCP tool."""
...
from nomad_ai.agent import root_agent
...
async def get_agent_async():
    ...
    print("\nInserting Airbnb MCP tools into Nomad-AI...")
...
async def async_main(question):
    """Executes one turn of the nomad-ai agents with a query that would trigger the MCP tool."""
    session_service = InMemorySessionService()
    artifacts_service = InMemoryArtifactService()
    session = session_service.create_session(
        state={}, app_name="nomad-ai", user_id="traveler0115"
    )
